package com.jrapid.common.collection;
...
/**
 * A hashtable-based <tt>Map</tt> implementation with <em>weak keys</em>. An entry in a <tt>WeakHashMap</tt> will
 * automatically be removed when its key is no longer in ordinary use. More precisely, the presence of a mapping for a
 * given key will not prevent the key from being discarded by the garbage collector, that is, made finalizable,
 * finalized, and then reclaimed. When a key has been discarded its entry is effectively removed from the map, so this
 ...
 * <tt>HashMap</tt> class, and has the same efficiency parameters of <em>initial capacity</em> and <em>load factor</em>.
 * 
 * <p>
 * Like most collection classes, this class is not synchronized. A synchronized <tt>WeakHashMap</tt> may be constructed
 * using the {@link Collections#synchronizedMap Collections.synchronizedMap} method.
 * 
 * <p>
 * This class is intended primarily for use with key objects whose <tt>equals</tt> methods test for object identity
 ...
 * 
 * <p>
 * The behavior of the <tt>WeakHashMap</tt> class depends in part upon the actions of the garbage collector, so several
 * familiar (though not required) <tt>Map</tt> invariants do not hold for this class. Because the garbage collector may
 * discard keys at any time, a <tt>WeakHashMap</tt> may behave as though an unknown thread is silently removing entries.
 * In particular, even if you synchronize on a <tt>WeakHashMap</tt> instance and invoke none of its mutator methods, it
 * is possible for the <tt>size</tt> method to return smaller values over time, for the <tt>isEmpty</tt> method to
 * return <tt>false</tt> and then <tt>true</tt>, for the <tt>containsKey</tt> method to return <tt>true</tt> and later
 * <tt>false</tt> for a given key, for the <tt>get</tt> method to return a value for a given key but later return
 * <tt>null</tt>, for the <tt>put</tt> method to return <tt>null</tt> and the <tt>remove</tt> method to return
 * <tt>false</tt> for a key that previously appeared to be in the map, and for successive examinations of the key set,
 * the value collection, and the entry set to yield successively smaller numbers of elements.
 * 
 * <p>
 * Each key object in a <tt>WeakHashMap</tt> is stored indirectly as the referent of a weak reference. Therefore a key
 * will automatically be removed only after the weak references to it, both inside and outside of the map, have been
 * cleared by the garbage collector.
 * 
 * <p>
 * <strong>Implementation note:</strong> The value objects in a <tt>WeakHashMap</tt> are held by ordinary strong
 ...
 * deal with this is to wrap values themselves within <tt>SoftReferences</tt> before inserting, as in:
 * <tt>m.put(key, new SoftReference(value))</tt>, and then unwrapping upon each <tt>get</tt>.
 * 
 * <p>
...
public class SoftHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V> {
     ...
     * change the number of mappings in the map or otherwise modify its internal structure (e.g., rehash). This field is
     ...
     * mapping from key <tt>k</tt> to value <tt>v</tt> such that <code>(key==null ?  k==null :
    ...
    /**
     * The entries in this hash table extend SoftReference, using its main ref field as the key.
     */
